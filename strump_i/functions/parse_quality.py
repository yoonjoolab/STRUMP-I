...
class ParseQuality:
    ...
    def __init__(self, input_dir, output_dir):
        '''
            Initiation.
            need value : input_dir, output_dir
        '''
        self.indir = input_dir
        self.outdir = output_dir
        # self.allele = allele

    def parse_matching_score(self, lines: list):
        '''
            Line parser for PDB ID, and matching scores.
        '''
        tgt_lines = [x for x in lines if "find_template" in x and "Matched" in x]
        if len(tgt_lines):
            tgt_line = tgt_lines[0]
            tokens = [x for x in tgt_line.split(",") if "Matched" in x]
            pdb, blosum, blosum_ratio, pam, pam_ratio = [x.split(": ")[1] for x in tokens]
            pam_ratio = pam_ratio.strip()
            returndict = {"pdb_id": pdb,
                        "HLA_BLOSUM_score": blosum, "HLA_BLOSUM_ratio": blosum_ratio,
                        "Peptide_PAM": pam, "Peptide_PAM_ratio": pam_ratio}
        else:
            returndict = {"pdb_id": "",
                        "HLA_BLOSUM_score": '', "HLA_BLOSUM_ratio": "",
                        "Peptide_PAM": '', "Peptide_PAM_ratio": ''}
        return returndict
    ...
    def parse_all_log(self):
        ...
        # column reordering
        column_order = [
            'Pdb', 'pdb_id', 'allele', 'peptide',
            'HLA_BLOSUM_score', 'HLA_BLOSUM_ratio',
            'Peptide_PAM', 'minimized_RMS',
    ...
    def __call__(self):
        ...
        save_fn = os.path.join(self.outdir, "quality_data.csv")
